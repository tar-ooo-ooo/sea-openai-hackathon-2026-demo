...
請在已依序完成 `01-infrastructure.md` 到 `11-chat-workflow-links.md` 的 repo 上，將原本保存在 localStorage 的本機 Demo 業務資料改由同一個 Node server 保存到 Git ignore 的 `/db/*.txt`。目前登入身份仍保存在 sessionStorage。開始前完整閱讀 `prompts/00-overview.md`、`AGENTS.md`、`server/index.js`、`src/services/data.ts`、`src/App.tsx` 與既有測試。
...
- server 尚未建立某份文字檔時，可將同 key 的舊 localStorage JSON 一次性複製到 server；完成後所有正式讀寫只使用資料 API。
...
```bash
node --test server/services/chat-instructions.test.js server/services/application-storage.test.js server/services/file-store.test.js
