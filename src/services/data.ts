/**
 * 讀取指定 server 資料集的 JSON 資料。
 * @template T 資料型別。
 * @param storeName server 資料集名稱。
 * @param fallback 找不到資料時的預設值。
 * @returns 解析後的資料或預設值。
 */
export async function loadData<T>(storeName: string, fallback: T): Promise<T> {
  try {
    const _response = await fetch(`/api/data/${storeName}`)
    const _result = (await _response.json()) as { exists?: unknown; data?: unknown }

    if (!_response.ok) return fallback
    return _result.exists ? _result.data as T : fallback
  } catch {
    return fallback
  }
}
...
/**
 * 將資料序列化後儲存至指定 server 資料集。
 * @param storeName server 資料集名稱。
 * @param value 要儲存的資料。
 * @returns 是否成功寫入。
 */
export async function saveData(storeName: string, value: unknown): Promise<boolean> {
  try {
    const _response = await fetch(`/api/data/${storeName}`, {
      method: 'PUT',
...
async function _loadUserStore(): Promise<_UserStore> {
  ...
  const _stored = await loadData<unknown>(_userStoreName, { version: 1, users: [] })
...
// 本機帳號資料集名稱。
const _userStoreName = 'users'
// 初步個人資料集名稱。
const _profileStoreName = 'profiles'
...
export async function loadProfile(): Promise<Profile> {
  ...
  const _storedProfile = await loadData<Profile | _LegacyProfile | unknown>(_profileStoreName, _profileFallback)
...
export async function saveProfile(profile: Profile): Promise<boolean> {
  return saveData(_profileStoreName, profile)
}
...
export async function registerUser(nationalId: string, password: string): Promise<'registered' | 'exists' | 'invalid' | 'storage-error'> {
  ...
  const _isSaved = await saveData(_userStoreName, {
    version: 1,
...
export async function loadChatMessages(nationalId: string): Promise<ChatMessage[]> {
  // 讀取可在未來遷移的版本化聊天資料。
  const _store = await loadData<_ChatHistoryStore | _LegacyChatHistoryStore>(_chatHistoryStoreName, _chatHistoryFallback)
  // 取得目前身份的聊天備份。
...
export async function saveChatMessages(nationalId: string, messages: ChatMessage[]): Promise<boolean> {
  ...
  return saveData(_chatHistoryStoreName, {
    version: 3,
...
async function _loadApplicationPackageStore(): Promise<_ApplicationPackageStore> {
  // 讀取可安全失敗的版本化本機資料。
  const _stored = await loadData<unknown>(_applicationPackageStoreName, _applicationPackageFallback)
  ...
  if (_store.version === 1) {
    ...
    await saveData(_applicationPackageStoreName, _migrated)
    return _migrated
  }
  ...
  if (_needsRepair) await saveData(_applicationPackageStoreName, _normalizedStore)
